...
plot <-
  ...
  geom_rect(data=d, mapping=aes(xmin=x1, xmax=x2, ymin=y1, ymax=y2, fill=as.factor(color)), 
            color="black") +
  scale_fill_manual(values=moma.colors("Flash", 10)[2:10]) +
  theme_void() +
  theme(plot.margin = unit(c(bottom=-.5, left=-.5, top=-.5, right=-.5),"cm"),
    legend.position = "none",
    panel.background = element_rect(linewidth=10),
    plot.background = element_rect(fill=moma.colors("Flash", 10)[1], linewidth = 10, color="black")
  )
